...
# dlib face detector and shape predictor initialization paths
detector = dlib.get_frontal_face_detector()
predictor_path = "shape_predictor_68_face_landmarks.dat"  # update path accordingly
predictor = dlib.shape_predictor(predictor_path)

# Eye indices for dlib 68-point model (updated)
...
def compute_ear_dlib(landmarks, idx):
    p1 = (landmarks[idx[0]].x, landmarks[idx[0]].y)
    p2 = (landmarks[idx[1]].x, landmarks[idx[1]].y)
    p3 = (landmarks[idx[2]].x, landmarks[idx[2]].y)
    p4 = (landmarks[idx[3]].x, landmarks[idx[3]].y)
    p5 = (landmarks[idx[4]].x, landmarks[idx[4]].y)
    p6 = (landmarks[idx[5]].x, landmarks[idx[5]].y)
...
def draw_landmarks_dlib(frame, landmarks, indices, color=(0, 255, 0)):
    for idx in indices:
        x, y = landmarks[idx].x, landmarks[idx].y
        cv2.circle(frame, (x, y), 2, color, -1)

# ---------------- robust extras ----------------
def eye_bbox_from_landmarks_dlib(landmarks, idx, img_w, img_h, pad=3):
...
def eye_clf_score(eye_gray_resized_24x24):
    ...
    if LINEAR_MODEL is not None:
        ...
        return float(np.dot(w, v) + b)  # model score
    ...
    def run(self):
        ...
        try:
            while not self._stop:
                ...
                faces = detector(gray)
                if faces:
                    face = faces[0]  # Process first face only
                    shape = predictor(gray, face)
                    landmarks = shape.parts()

                    # center check
                    nx, ny = landmarks[30].x, landmarks[30].y  # Nose tip point
                    center = (w // 2, h // 2)
                    ...
                    if face_status == "IN":
                        leftEAR = compute_ear_dlib(landmarks, LEFT_EYE_IDX)
                        rightEAR = compute_ear_dlib(landmarks, RIGHT_EYE_IDX)
                        avgEAR = (leftEAR + rightEAR) / 2.0

                        lx1, ly1, lx2, ly2 = eye_bbox_from_landmarks_dlib(
                            landmarks, LEFT_EYE_IDX, w, h, pad=4
                        )
                        rx1, ry1, rx2, ry2 = eye_bbox_from_landmarks_dlib(
                            landmarks, RIGHT_EYE_IDX, w, h, pad=4
                        )
                        eye_metrics = []
                        for (x1, y1, x2, y2) in ((lx1, ly1, lx2, ly2), (rx1, ry1, rx2, ry2)):
                            ...
                            if crop.size == 0:
                                continue
                            gray_eye = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)
                            gray_eye = clahe_gray(gray_eye)
                            pr = pupil_dark_ratio(gray_eye)
                            small = cv2.resize(gray_eye, (24, 24), interpolation=cv2.INTER_AREA)
                        ...
                        if elapsed_time <= baseline_duration:
                            ...
                            top_open = sorted(calibration_ears)[
                                -max(3, int(0.7 * len(calibration_ears))) :
                            ]
                        ...
                        # draw landmarks
                        draw_landmarks_dlib(orig, landmarks, LEFT_EYE_IDX, color=(0, 255, 0))
                        draw_landmarks_dlib(orig, landmarks, RIGHT_EYE_IDX, color=(0, 255, 0))
                    else:
                        avgEAR = 0.0
                        eye_open = False
                ...
                circle_color = (0, 255, 0) if face_open else (0, 0, 255)
                ...
                if self.show_window:
                    ...
                    y_offset = 30
                    line_h = 28
                    cv2.putText(display_frame, f"Time: {elapsed_time:.1f}s", (30, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, text_color, 2)
                    y_offset += line_h
                    cv2.putText(display_frame, f"EAR: {avgEAR:.3f}", (30, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, text_color, 2)
                    y_offset += line_h
                    cv2.putText(display_frame, f"Eye State: {eye_state_text}", (30, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, text_color, 2)
                    y_offset += line_h
                    cv2.putText(display_frame, f"Blinks: {blink_count}", (30, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, text_color, 2)
                    y_offset += line_h
                    cv2.putText(display_frame, f"Status: {status}", (30, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, result_color, 2)
                    y_offset += line_h
                    cv2.putText(display_frame, f"Blink Rate: {blink_rate:.2f}/min", (30, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, text_color, 2)
                    y_offset += line_h
                ...
                if elapsed_time > baseline_duration:
                    if not baseline_collected:
                        personal_baseline_blinks = (baseline_blink_count / baseline_duration) * 60 or 7  # Default 7 if 0
