...
def get_users_csv_path():
    ...
    return os.path.join(folder_path, "user1.csv")
...
def run_blink_test(test_type, user_id, show_instructions=True):
    ...
    if not cap.isOpened():
        QMessageBox.warning(None, "Error", "Camera not accessible.")
...
class WelcomePage(QWidget):
    def __init__(self, stacked_widget):
        ...
        self.setStyleSheet("""
            QWidget {
                background: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:1, stop:0 #ffffff, stop:1 #2193b0);
            }
        """)
...
class UserChoicePage(QWidget):
    def __init__(self, stacked_widget):
        ...
        self.setStyleSheet("""
            QWidget { background: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:1, stop:0 white, stop:1 #2193b0); }
        """)
        ...
        for btn in [existing_user_btn, new_user_btn]:
            ...
            btn.setStyleSheet("""
                QPushButton { background-color: #0b3d91; color: white; border-radius: 15px; }
                QPushButton:hover { background-color: #062c6e; }
            """)
...
class EyeBlinkPage(QWidget):
    def __init__(self, stacked_widget):
        ...
        self.stacked_widget = stacked_widget
        self.setStyleSheet("""
            QWidget { background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #e0f7fa, stop:1 #80deea); }
        """)
        ...
        for btn in [btn_baseline, btn_testing, btn_random]:
            ...
            btn.setStyleSheet("""
                QPushButton { background-color: #1565c0; color: white; border-radius: 15px; }
    ...
    def open_baseline(self):
        ...
        QMessageBox.information(self, "Baseline Saved", f"Baseline {slot} saved.\nTotal blinks: {total_blinks}\nStatus: {final_status}")
    ...
    def open_testing(self):
        ...
        if total_blinks is None:
            return
        updates = {
            "Testing_TotalBlinks": str(total_blinks),
            "Testing_Timestamp": timestamp,
            "Testing_FinalStatus": final_status
        }
        update_user_row(user_id, updates)
        QMessageBox.information(self, "Testing Complete", f"Testing saved.\nTotal blinks: {total_blinks}\nStatus: {final_status}")
    ...
    def open_random(self):
        ...
        update_user_row(user_id, updates)
        QMessageBox.information(self, "Random Test Saved", f"Random test saved.\nTotal blinks: {total_blinks}\nStatus: {final_status}")
